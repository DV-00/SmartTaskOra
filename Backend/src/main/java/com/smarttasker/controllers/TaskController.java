...
public class TaskController {
    ...
    // NEW ENDPOINT: For status-only updates (used by drag-and-drop)
    @PreAuthorize("isAuthenticated()")
    @PutMapping("/{taskId}/status") // Example path: /api/tasks/{taskId}/status
    public ResponseEntity<Void> updateTaskStatus(@PathVariable Long taskId, @RequestBody String newStatusString) {
        try {
            // Convert the incoming string (e.g., "COMPLETED") to an enum
            TaskStatus newStatus = TaskStatus.valueOf(newStatusString.toUpperCase());
            int updatedRows = taskService.updateTaskStatus(taskId, newStatus);

            if (updatedRows > 0) {
                return ResponseEntity.ok().build(); // Success
            } else {
                return ResponseEntity.notFound().build(); // Task not found or not updated
            }
        } catch (IllegalArgumentException e) {
            // This catches if the newStatusString is not a valid TaskStatus enum name
            // FIX THIS LINE: Change .body("...") to .build()
